...
void woq_quantize(woq_config_param* p, woq_runtime_ctx* ctx) {
  ...
  *(ctx->output) = torch::empty(packedw.mSize, torch::kInt8);
  ...
  if (ctx->weight->scalar_type() == torch::kFloat32) {
    weight_ptr = ctx->weight->data_ptr<float>();
  } else if (ctx->weight->scalar_type() == torch::kBFloat16) {
    tmp_tensor = ctx->weight->to(torch::kFloat32);
    weight_ptr = tmp_tensor.data_ptr<float>();
  } else {
    TORCH_CHECK(false, "Qbits: unsupported input weight data_type in woq quantize, should be fp32/bf16.");
  }
  if (ctx->transpose) {
    launcher.mProB.packTransposeWeight(ctx->n, ctx->k, weight_ptr, ctx->k, &packedw,
                                       &dispatcher_utils::DefaultThreading);
  } else {
    launcher.mProB.packWeight(ctx->n, ctx->k, weight_ptr, ctx->n, &packedw, &dispatcher_utils::DefaultThreading);
  }
...
template <class GemmCore, template <class _T, JBLAS_ISA> class PrologueB,
...
void parse_launcher(woq_config_param* p, woq_runtime_ctx* ctx) {
  if constexpr (GemmCore::ISA == JblasAMX_INT8 || GemmCore::ISA == JblasAVX512_VNNI || GemmCore::ISA == JblasAVX_VNNI) {
    ...
    return execute_task<Launcher>(p, ctx);
  } else {
...
void parse_store(woq_config_param* p, woq_runtime_ctx* ctx) {
  ...
  if (p->dst_dt == dispatcher_utils::QBITS_FP32) {
    return parse_launcher<GemmCore, PrologueB, PrologueA, AlphaBetaProcessStoreFp32>(p, ctx);
  }
  if (p->dst_dt == dispatcher_utils::QBITS_BF16) {
    return parse_launcher<GemmCore, PrologueB, PrologueA, AlphaBetaProcessStoreBf16>(p, ctx);
  }
}
...
void parse_activation(woq_config_param* p, woq_runtime_ctx* ctx) {
  ...
  if (p->src_dt == dispatcher_utils::QBITS_FP32) {
    if constexpr (GemmCore::ISA == JblasAMX_INT8 || GemmCore::ISA == JblasAVX512_VNNI ||
      ...
      return parse_store<GemmCore, PrologueB, ActivationF32KBlockQuantize, dispatcher_utils::QBITS_FP32>(p, ctx);
    } else {
      if constexpr (GemmCore::ISA == JblasAMX_BF16) {
        return parse_store<GemmCore, PrologueB, ActivationConverterFp32, dispatcher_utils::QBITS_FP32>(p, ctx);
      } else {
        return parse_store<GemmCore, PrologueB, ActivationBase, dispatcher_utils::QBITS_FP32>(p, ctx);
      }
    }
  }
  if (p->src_dt == dispatcher_utils::QBITS_BF16) {
    if constexpr (GemmCore::ISA == JblasAMX_INT8 || GemmCore::ISA == JblasAVX512_VNNI ||
                  GemmCore::ISA == JblasAVX_VNNI) {
      return parse_store<GemmCore, PrologueB, ActivationBf16KBlockQuantize, dispatcher_utils::QBITS_BF16>(p, ctx);
    } else {
      if constexpr (GemmCore::ISA == JblasAMX_BF16) {
        return parse_store<GemmCore, PrologueB, ActivationBase, dispatcher_utils::QBITS_BF16>(p, ctx);
      } else {
        return parse_store<GemmCore, PrologueB, ActivationConverterBf16, dispatcher_utils::QBITS_BF16>(p, ctx);
      }
    }
  }
}
...
void parse_weight(woq_config_param* p, woq_runtime_ctx* ctx) {
  ...
  if (p->weight_type == "int8" || p->weight_type == "int4_clip" || p->weight_type == "int4_fullrange") {
    return parse_activation<GemmCore, WeightKBlockNInteger>(p, ctx);
  }
  if (p->weight_type == "nf4" || p->weight_type == "fp4_e2m1_bnb" || p->weight_type == "fp4_e2m1" ||
      p->weight_type == "fp8_e4m3" || p->weight_type == "fp8_e5m2") {
    if constexpr (GemmCore::ISA != JblasAMX_INT8 && GemmCore::ISA != JblasAVX512_VNNI && GemmCore::ISA != JblasAVX_VNNI)
      return parse_activation<GemmCore, WeightKBlockNFloat>(p, ctx);
  }
  TORCH_CHECK(false,
...
void parse_gemm_core_online(woq_config_param* p, woq_runtime_ctx* ctx) {
  ...
  if (p->compute_type == "int8") {
    if (dispatcher_utils::check_amx() && ctx->blocksize % jblas::gemm::ICoreRowNAmxint8KBlock<48, 16>::KTILE == 0) {
      return parse_weight<jblas::gemm::ICoreRowNAmxint8KBlock<48, 16>>(p, ctx);
    }
    if (dispatcher_utils::check_avx512_vnni() &&
        ctx->blocksize % jblas::gemm::ICoreRowNAvx512vnniKBlock<48, 4>::KTILE == 0) {
      return parse_weight<jblas::gemm::ICoreRowNAvx512vnniKBlock<48, 4>>(p, ctx);
    }
    if (dispatcher_utils::check_avx_vnni() && ctx->blocksize % jblas::gemm::ICoreRowNAvxvnniKBlock<48, 2>::KTILE == 0) {
      return parse_weight<jblas::gemm::ICoreRowNAvxvnniKBlock<48, 2>>(p, ctx);
    }
  ...
  if (p->compute_type == "fp32") {
    if (dispatcher_utils::check_avx512f()) {
      return parse_weight<jblas::gemm::SCoreRowNAvx512f<48, 8>>(p, ctx);
    }
    if (dispatcher_utils::check_avx2()) {
      return parse_weight<jblas::gemm::SCoreRowNAvx2<24, 4>>(p, ctx);
    }
  ...
  if (p->compute_type == "bf16") {
    if (dispatcher_utils::check_amx()) {
      return parse_weight<jblas::gemm::HCoreRowNAmxbf16<64, 16>>(p, ctx);
    }
...
void parse_gemm_core_offline(woq_config_param* p, woq_runtime_ctx* ctx) {
  ...
  if (CType == uint32_t(jblas::gemm::CompType::COMP_FP32)) {
    ...
    if (NTile == jblas::gemm::SCoreRowNAvx2<24, 4>::NTILE && dispatcher_utils::check_avx2()) {
      return parse_weight<jblas::gemm::SCoreRowNAvx2<24, 4>>(p, ctx);
    }
  }
...
void parse_gemm_core(woq_config_param* p, woq_runtime_ctx* ctx) {
  if (p->task == WOQ_QUANTIZE)
    return parse_gemm_core_online(p, ctx);
  else
    return parse_gemm_core_offline(p, ctx);
}

void dispatch_woq_task(woq_config_param* p, woq_runtime_ctx* ctx) { return parse_gemm_core(p, ctx); }
